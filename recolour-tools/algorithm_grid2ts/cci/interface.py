...
from library_cci.grid import CCI025Cellgrid
    ...
    def read(self, timestamp=None):
        ...
        for parameter, variable in dataset.variables.items():
            if parameter in param_names:
                ...
                param_data = np.ma.getdata(param_data.filled())

                param_data_temp = np.zeros([param_data.shape[0], param_data.shape[1], param_data.shape[2]])
                for i in range(0,param_data.shape[0]):
                    param_data_temp[i, :, : ] = np.flipud(param_data[i, : ,:])

                param_data = np.concatenate((self.fill_values, param_data_temp.flatten()))

                #param_data_reshape = param_data.reshape((720, 1440))
                #plt.figure(1)
                #plt.imshow(param_data_reshape)
                #plt.show()
...
class CCI_SM_v042_025Ds(MultiTemporalImageBase):
    ...
    def __init__(self, data_path, parameter='sm', array_1D=False):
        ...
        filename_templ = "ESACCI-SOILMOISTURE-L3S-SSMV-PASSIVE-{datetime}-fv04.7.nc"
