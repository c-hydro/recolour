...
script_name='RECOLOUR - DOWNLOADER - SOIL MOISTURE HMC - NRT REALTIME'
		...
		if [ "${data_active_step}" = true ] ; then
			...
			for day in $(seq ${data_days_step} -1 0); do
				...
				if [ "${data_flag_download}" = true ] ; then
					...
					# rsync command
					if sshpass -p ${machine_pwd} rsync -avr ${machine_usr}@${machine_url}:${data_folder_src_step}/${data_file_src_step} ${data_folder_dst_step}/${data_file_dst_step} --info=progress2 > /dev/null 2>&1; then
				 		echo " ... DONE!"
					else
