...
def join_dataset_obj(dset_collections_in, time_collections_in, ancillary_collections_in,
                     max_dist_k1=25000, max_dist_k2=15000, max_dist_anc=25000,
    ...
    if dset_anc is not None:
        ...
        geo_x_k1_raw, geo_y_k1_raw, grid_k1 = get_dataset_geo(dset_k1)
        geo_x_k2_raw, geo_y_k2_raw, grid_k2 = get_dataset_geo(dset_k2)
        ...
        dset_k1_filter = filter_datasets_obj(dset_obj_in=dset_k1, idxs_obj=idxs_k1_filter)
        dset_k2_filter = filter_datasets_obj(dset_obj_in=dset_k2, idxs_obj=idxs_k2_filter)
...
def create_composite_dset(dset_ref, dset_k1, dset_k2):
    ...
    values_ref = dset_ref['soil_moisture_ref']
    values_k1 = dset_k1['soil_moisture_k1']
    values_k2 = dset_k2['soil_moisture_k2']
    ...
    idxs_ref_finite = np.squeeze(np.argwhere(dset_ref['soil_moisture_ref'] >= 0)).tolist()
    idxs_k1_finite = np.squeeze(np.argwhere(dset_k1['soil_moisture_k1'] >= 0)).tolist()
    idxs_k2_finite = np.squeeze(np.argwhere(dset_k2['soil_moisture_k2'] >= 0)).tolist()
