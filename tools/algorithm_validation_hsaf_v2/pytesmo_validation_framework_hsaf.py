...
from lib_utils_generic import manage_file_workspace, dump_file_workspace
...
class Validation(object):
        ...
        *args,
        ...
        for gpi_info in zip(gpis, lons, lats, *args):
            ...
            if workspace is not None:
                file_tmpl = workspace['file_name_tmpl']
                update_wks, active_wks = workspace['workspace_update'], workspace['workspace_active']

                if active_wks:

                    gpi_id, gpi_lon, gpi_lat = gpi_info[0], gpi_info[1], gpi_info[2]

                    grid_ref = self.data_manager.reference_grid
                    cell_id = grid_ref.gpi2cell(gpi_id)

                    file_wks = manage_file_workspace(
                        file_tmpl, file_gpi=gpi_id, file_cell=cell_id)

                    if os.path.exists(file_wks):
                       if update_wks:
                           if os.path.exists(file_wks):
                               os.remove(file_wks)
                       else:
            ...
            try:
                ...
                '''
                # debug - check gpi data and results
                save_validation_data(file_path=f"debug/gpi_{gpi_info[0]}_v2.json",
                                     df_dict=df_dict, matched_data=matched_data,
                                     result=result, used_data=used_data
                                     )
                '''

                if file_wks is not None:

                    attrs_wks = {'gpi': gpi_info[0], 'longitude': gpi_info[1], 'latitude': gpi_info[2]}
                    result_wks = {'matched_data': matched_data, 'result': result, 'used_data': used_data}

                    dump_file_workspace(file_name=file_wks, datasets=df_dict,
                                        attributes=attrs_wks, results=result_wks)

            except Exception as e:
                if handle_errors == 'raise':
                    raise e
                elif handle_errors == "ignore":
                    logging.error(f' ===> Datasets are not defined for point "{gpi_info}": "{e}"')
    ...
    def perform_validation(
        ...
        for n, k in self.metrics_c:
            ...
            # iterate over datasets and names
...
# ----------------------------------------------------------------------------------------------------------------------
# method to save validation debug collections in one human-readable JSON file
def save_validation_data(
        ...
        used_data=None):
    """
    Save validation debug data into one human-readable JSON file.

    Notes
    -----
    - All dictionary keys are converted to strings.
    - Tuple keys are flattened using "__".
    - NaN values are converted to None.
    - pandas/numpy objects are converted to JSON-compatible values.
    """
    ...
    def sanitize_json(obj):
        ...
        # debug info
        print(f"[DEBUG] Sanitizing object type: {type(obj)}")
        ...
        if isinstance(obj, dict):
            ...
            for key, value in obj.items():
                ...
                if isinstance(key, tuple):
                    key_clean = "__".join(map(str, key))
                    print(f"[DEBUG] Tuple key converted: {key} --> {key_clean}")

                else:
                    key_clean = str(key)

                obj_clean[key_clean] = sanitize_json(value)
        ...
        if isinstance(obj, pd.DataFrame):
            ...
            print(f"[DEBUG] DataFrame detected with shape: {obj.shape}")

            df_clean = obj.astype(object).where(obj.notnull(), None)

            return {
                "columns": [str(col) for col in df_clean.columns],
                "index": [str(idx) for idx in df_clean.index],
                "data": sanitize_json(df_clean.values.tolist())
            }

        # series objects
        if isinstance(obj, pd.Series):

            print(f"[DEBUG] Series detected with length: {len(obj)}")

            obj_clean = obj.astype(object).where(obj.notnull(), None)

            return {
                "index": [str(idx) for idx in obj_clean.index],
                "data": sanitize_json(obj_clean.tolist())
            }

        # numpy array objects
        if isinstance(obj, np.ndarray):

            print(f"[DEBUG] ndarray detected with shape: {obj.shape}")
    ...
    print(f"[DEBUG] Saving validation data to: {file_path}")

    # organize payload
    payload = {
        "metadata": {
            "created_at": datetime.now().isoformat(),
            "matched_data": sanitize_json(matched_data),
            "result": sanitize_json(result),
            "used_data": sanitize_json(used_data)
        },
        "df_dict": sanitize_json(df_dict)
    }

    print("[DEBUG] Payload successfully sanitized")
    ...
    print("[DEBUG] Validation data successfully saved")
# ------------------------------------------------------------------------------------------------------------------
