...
"""
RECOLOUR - PROCESSING TOOLS - File Transfer
...
def main():
    ...
    if 'ancillary' in list(settings_data.keys()):
        ...
        ancillary_raw = {}; ancillary_raw['ancillary'] = {}
            ...
            for tag_value in ancillary_raw['tag_name_list']:
                ...
                if file_list_src_def:
                    ...
                    for file_path_src_step, file_path_dst_step in zip(file_list_src_def, file_list_dst_def):
                        ...
                        log_stream.info(
                            adjust_comment(' ------> Transfer source datasets "' + file_path_src_step +
                                           '" to destination datasets "' + file_path_dst_step + '" ... '))
                        ...
                        if check_dataset_source:
                            ...
                            execute_command(
                                ...
                                command_type='Transfer source datasets "' +
                                             file_path_src_step + '" to destination datasets "' +
                                             file_path_dst_step + '"')
                            ...
                            if method_cmd_uncompress_file is not None:
                                execute_command(
                                    ...
                                    command_type='Extract compressed destination datasets "' + file_path_dst_step + '"')
                            ...
                            if method_cmd_remove_file is not None:
                                ...
                                command_find_file_code = execute_command(
                                    ...
                                    command_type='Find compressed destination datasets "' + file_path_dst_step + '"')

                                if command_find_file_code:
                                    execute_command(
                                        method_cmd_remove_file, command_prefix=' -------> ',
                                        command_type='Remove compressed destination datasets "' + file_path_dst_step + '"')

                            # Transfer datasets from local to remote
                            log_stream.info(
                                adjust_comment(' ------> Transfer source datasets "' + file_path_src_step +
                                               '" to destination datasets "' + file_path_dst_step + '" ... DONE'))

                        else:
                            log_stream.info(
                                adjust_comment(' ------> Transfer source datasets "' + file_path_src_step +
                                            '" to destination datasets "' + file_path_dst_step + '" ... SKIPPED. '
