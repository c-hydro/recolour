...
from lib_img2ts_hmc import Img2Ts
...
def reshuffle(product,
    ...
    # get data reference in the period
    first_date = mkdate(start_date.strftime("%Y-%m-%d 23:%M"))
    try:
        data = dataset_grid_driver.read(first_date)
    except IOError:
        logging.error(' ===> I/O error.')
        logging.info(' -------> Analyze time "' + first_date.isoformat() + '" ... FAILED. SKIPPING ...')
        raise SystemExit
...
def parse_args(args):
    ...
    parser = argparse.ArgumentParser(
        description="Convert HMC data to time series format."
    )
...
def main(args):
    ...
    # info start
    logging.info(
        ' ---> Convert HMC datasets from {} to {} into folder {} ... '.format(
    ...
    # info end
    logging.info(
        ' ---> Convert HMC datasets from {} to {} into folder {} ... DONE'.format(
            args.start.isoformat(), args.end.isoformat(), args.dataset_ts_root
        )
    )
# ----------------------------------------------------------------------------------------------------------------------
