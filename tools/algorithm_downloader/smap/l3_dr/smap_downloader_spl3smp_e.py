...
def process_cmr(filename_obj_source, fileroot_obj_source,
    ...
    for (fn_source_time, fp_source), fr_source_list, fp_anc_global_list, fp_anc_domain_list, \
        ...
        layer_dict_AM = {'time': 'AM', 'fp_outcome_step': None, 'data': [], 'metadata': []}
        layer_dict_PM = {'time': 'PM', 'fp_outcome_step': None, 'data': [], 'metadata': []}
        ...
        for fr_source_step, fp_anc_global_step, fp_anc_domain_step, fp_outcome_step, var_step in zip(
            ...
            if var_step=='AM/soil_moisture': layer_dict_AM['fp_outcome_step'] = fp_outcome_step
            elif var_step=='PM/soil_moisture_pm': layer_dict_PM['fp_outcome_step'] = fp_outcome_step
            ...
            reproject_file_tiff(file_name_in=fp_anc_global_step, file_name_out=fp_anc_domain_step,
                                file_wide_out=geo_wide, file_high_out=geo_high, file_geotrans_out=geo_geotrans, file_proj_out=geo_proj,
                                bbox=[geo_min_x, geo_max_y, geo_max_x, geo_min_y], original_grid=False)
            ...
            # ----------------------------------------------
            smap_domain_dset, smap_domain_proj, smap_domain_geotrans, smap_domain_data,\
                smap_domain_wide, smap_domain_high = read_file_tiff(fp_anc_domain_step)
            # ----------------------------------------------
            ...
            # Mask negative and inf values (<0 and >100)
            smap_domain_masked_1d[smap_domain_masked_1d<=0.] = np.nan
            # smap_domain_masked_1d[smap_domain_masked_1d>100.] = np.nan
            smap_domain_masked_1d = np.nan_to_num(smap_domain_masked_1d, nan=-9999.)
            ...
            if 'AM' in var_step:
                var = var_step.split('/')[-1]
                layer_dict_AM['data'].append(smap_domain_masked_2d)
                layer_dict_AM['metadata'].append({'description':var})
            elif 'PM' in var_step:
                var = var_step.split('/')[-1]
                var = var[:-3] # cut off the _pm at the end
                layer_dict_PM['data'].append(smap_domain_masked_2d)
                layer_dict_PM['metadata'].append({'description':var})

            # set tiff parameters
            if not all([image_high, image_proj, image_wide, image_geotrans]):
                image_high = smap_domain_high
                image_wide = smap_domain_wide
                image_proj = smap_domain_proj
                image_geotrans = smap_domain_geotrans

            # Info
            logging.info(' -----> Process variable: ' + var_step + ' ... DONE')

        # Once all the tiffs have been read, write all of them, store data,
        # write a multi-layer tiff with additional sm layer [sta.deg.] masked for quality

        # Write domain tiff file in epsg:4326
        for dict in [layer_dict_AM, layer_dict_PM]:
            logging.info(' ------> Saving ' + dict['fp_outcome_step'] + ' ... ')

            # Create soil_moisture_qual layer
            idx_sm = dict['metadata'].index({'description':'soil_moisture'})
            soil_moisture_qual = dict['data'][idx_sm].copy()
            idx_flag = dict['metadata'].index({'description':'retrieval_qual_flag'})
            retrieval_qual_flag = dict['data'][idx_flag].copy()
            low_quality = np.logical_and(retrieval_qual_flag != 0, retrieval_qual_flag != 8)
            soil_moisture_qual[low_quality] = -9999.

            dict['data'].append(soil_moisture_qual)
            dict['metadata'].append({'description':'soil_moisture_qual'})

            write_file_tiff(dict['fp_outcome_step'], dict['data'], dict['metadata'], image_wide,
                        image_high, image_geotrans, image_proj, flip=False)
            logging.info(' ------> Saving ' + dict['fp_outcome_step'] + ' ... DONE')
...
def set_data_source(file_id, filename_url,
    ...
    for filename_url_step in filename_url:
        ...
        if filename_url_step.endswith(filename_suffix):
            ...
            match_time = re.search(r'\d{4}\d{2}\d{2}', filename_url_step)
...
def reproject_file_tiff(file_name_in, file_name_out,
                        file_wide_out, file_high_out, file_geotrans_out, file_proj_out, bbox=None,
                        original_grid=False):
    """Reproject the tiff file on the provided grid.

    If original_grid=False, use grid in geo_file. No need for bbox.
    If original_grid=True, use grid of product, but you need to provide a bbox.
    ...
    if original_grid:
        if bbox is None:
            logging.warning(' ===> Bounding box coordinates are set to default global coordinates.')
            # bbox must be [min_x, max_y, max_x, min_y]
            bbox = [-180, 90, 180, -90]

        # define geo-transformation which preserves original pixel size
        meters_to_degrees = 1 / 111320.0
        x_size_in = dset_geotrans_in[1]; x_size_out = meters_to_degrees * x_size_in
        y_size_in = dset_geotrans_in[5]; y_size_out = meters_to_degrees * y_size_in
        min_x, max_y, max_x, min_y = bbox
        dset_geotrans_out = (min_x, x_size_out, 0.0, max_y, 0.0, y_size_out)
        dset_wide_out = round(abs((max_x-min_x)/x_size_out))
        dset_high_out = round(abs((max_y-min_y)/y_size_out))
    else:
        dset_wide_out = file_wide_out
        dset_high_out = file_high_out
        dset_geotrans_out = file_geotrans_out


    dset_tiff_out = gdal.GetDriverByName('GTiff').Create(
        file_name_out, dset_wide_out, dset_high_out, 1, gdalconst.GDT_Float32)
    dset_tiff_out.SetGeoTransform(dset_geotrans_out)
...
def main():
    ...
    geo_proj, geo_geotrans, geo_data, geo_wide, geo_high, geo_min_x,\
            geo_max_y, geo_max_x, geo_min_y, geo_mask_idx = read_file_geo(
            os.path.join(geo_settings['folder'], geo_settings['filename']))
