...
class BaseImage(ImageBase):
    ...
    def read(self, timestamp=None):
        ...
        for i, parameter in zip(range(n_bands), self.parameters):
            param_metadata, param_data = {}, {}
            data_raw = dataset.read(i+1)
            data_values = data_raw[:,:] # np.float64(data_raw[0, :, :])
...
class smap_ds(MultiTemporalImageBase):
    ...
    def tstamps_for_daterange(self, start_date, end_date):
        ...
        img_offsets = np.array([timedelta(hours=step) for step in range(start_step, end_step +1, 1)])
