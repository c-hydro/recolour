...
def set_time_info(time_run_args=None, time_run_file=None, time_format='%Y-%m-%d %H:$M',
    ...
    if (time_run_file_start is None) and (time_run_file_end is None):
        ...
        if time_period > 1:
            ...
            time_range_arr = pd.date_range(end=time_day_start, periods=1, freq=time_frequency)
            time_range_start = time_range_arr[0]
            time_range_end = deepcopy(time_day_end)
        elif time_period == 0:
        ...
        if time_run_file_start == 'CONFIGURE_BY_TIME_NOW' and time_run_file_end == 'CONFIGURE_BY_TIME_NOW':
            ...
            time_now, time_range_start, time_range_end = time_period_run, time_period_start, time_period_end
...
def set_timedelta_min(time_start, time_end, time_delta_min='24H', time_rounding='D'):
    time_delta_upd = None
