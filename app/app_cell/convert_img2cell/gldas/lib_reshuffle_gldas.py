...
from lib_utils_gldas import read_obj, write_obj

from lib_img2cell_gldas import Img2Ts
from lib_interface_gldas import gldas_ds
from lib_grid_gldas import load_grid, cell_grid, subgrid4bbox
# ----------------------------------------------------------------------------------------------------------------------
...
def reshuffle(product,
              ...
              grid_path,
              parameters,
              file_name_tmpl_grid="GLDAS_NOAH025_3H.A{datetime}.021.nc4",
              datetime_format_grid="%Y%m%d.%H%M",
    ...
    if get_filetype(dataset_grid_root) == "netcdf":
        if input_grid is not None:
            logging.info(" ------> Land Grid is fit to GLDAS grid netCDF data")
            dataset_grid_driver = gldas_ds(
                product=product,
...
def main(args):
    ...
    # info start
    logging.info(
        ' ---> Convert GLDAS datasets from {} to {} into folder {} ... '.format(
    ...
    # info end
    logging.info(
        ' ---> Convert GLDAS datasets from {} to {} into folder {} ... DONE'.format(
            args.start.isoformat(), args.end.isoformat(), args.dataset_ts_root
        )
    )
# ----------------------------------------------------------------------------------------------------------------------
