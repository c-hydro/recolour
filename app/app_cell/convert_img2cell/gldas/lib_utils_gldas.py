...
def create_file_grid(grid_path, data_path, data_ext='.nc4', grid_update=True):
    ...
    if not os.path.exists(grid_path):
        ...
        if data_ext == '.tif' or data_ext == '.tiff':
            ...
            file_values, file_geo_x_1d, file_geo_y_1d = read_file_nc(
                file_path, var_name_data='SoilMoi0_10cm_inst', var_lim_max=100, var_lim_min=0)
        else:
...
def read_file_nc(file_name, var_name_data='SoilMoi0_10cm_inst', var_name_geo_x='lon', var_name_geo_y='lat',
                 var_lim_min=None, var_lim_max=None):

    # example variables = "SoilMoi0_10cm_inst", "SoilTMP0_10cm_inst", "SWE_inst"
    ...
    tmp_max, tmp_min = np.nanmax(tmp_values), np.nanmin(tmp_values)
    if var_lim_max is None:
        var_lim_max = tmp_max
    if var_lim_min is None:
        var_lim_min = tmp_min
    ...
    file_values[(file_values >= var_lim_min) & (file_values <= var_lim_max)] = 1
    file_values[file_values < var_lim_min] = 0
    file_values[file_values > var_lim_max] = 0
    ...
    plt.figure(); plt.imshow(tmp_geo_y_2d); plt.colorbar()
