...
def select_origin_grid(data_path: str, data_origin: (str, None) = None, default_origin: str = '.tif') -> str:
    ...
    if data_origin is None:
        ...
        for root, dirs, files in os.walk(data_path):
            for file in files:
                ...
                if extension in ['.tif', '.tiff', '.nc', '.nc4', '.netcdf']:
                    break
...
def create_file_grid(grid_path, data_path, data_ext='.tif', grid_update=True):
    ...
    logging.info(' ---> Create grid reference for static datasets ... ')
    ...
    if not os.path.exists(grid_path):
        ...
        if file_path is None:
            ...
            raise RuntimeError('Grid is needed by the procedure to convert files from grid to cell format')
        ...
        logging.info(' ---> Create grid reference for static datasets ... DONE')

    else:
        logging.info(' ---> Create grid reference for static datasets ... PREVIOUSLY DONE')
# ----------------------------------------------------------------------------------------------------------------------
...
def read_file_tiff(file_name):
    ...
    file_bounds = file_dset.bounds
    file_res = file_dset.res
    file_data = file_dset.read()

    file_values = np.float32(file_data[0, :, :])

    if file_dset.nodata is not None:
        file_values[file_values == file_dset.nodata] = np.nan
    ...
    file_geo_x_1d = np.arange(
        file_center_left,
        file_center_right + np.abs(file_res[0] / 2),
        np.abs(file_res[0]),
        float
    )

    file_geo_y_1d = np.flip(
        np.arange(
            file_center_bottom,
            file_center_top + np.abs(file_res[1] / 2),
            np.abs(file_res[1]),
            float
        ),
        axis=0
    ...
    file_geo_x_2d, file_geo_y_2d = np.meshgrid(
        file_geo_x_1d,
        file_geo_y_1d
    )

    file_geo_y_upper = file_geo_y_2d[0, 0]
    file_geo_y_lower = file_geo_y_2d[-1, 0]
...
def save_file_nc(
    ...
    dim_geo_x = file_geo_x_1d.shape[0]
    dim_geo_y = file_geo_y_1d.shape[0]

    file_dset = nc.Dataset(file_name, 'w', format='NETCDF4')

    file_dset.createDimension(var_geo_y, dim_geo_y)
    file_dset.createDimension(var_geo_x, dim_geo_x)
    ...
    file_var_values = file_dset.createVariable(
        var_name,
        'f4',
        (
            var_geo_y,
