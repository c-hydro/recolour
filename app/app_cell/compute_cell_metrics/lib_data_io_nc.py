...
def write_file_cell(file_name,
    ...
    if 'time' in list(data_attrs.keys()):
    ...
    for var_name in file_var_list_data:
        ...
        if isinstance(var_values[0], str):
            ...
            var_handle = file_handle.createVariable(varname=var_name, dimensions=(dim_obs,), datatype='str')
        elif isinstance(var_values[0], (float, np.floating)):
            var_data = var_values
            var_handle = file_handle.createVariable(varname=var_name, dimensions=(dim_obs,), datatype='f4')
        elif isinstance(var_values[0], (int, np.integer)):
            var_data = var_values
            var_handle = file_handle.createVariable(varname=var_name, dimensions=(dim_obs,), datatype='i4')
        elif isinstance(var_values[0], np.datetime64):
            if file_time_units is not None:
                ...
                alg_logger.error(' ===> Time units are not defined, but time variable is present in the variable datasets')
                raise IOError('Dump datasets failed due to the time units')
        else:
            alg_logger.error(' ===> Datasets format is not expected')
    ...
    for reg_name in file_var_list_registry:
        ...
        if isinstance(reg_values[0], str):
            ...
            if file_time_units is not None:
                ...
                alg_logger.error(' ===> Time units are not defined, but time variable is present in the registry datasets')
                raise IOError('Dump datasets failed due to the time units')
        else:
            alg_logger.error(' ===> Datasets format is not expected')
            raise IOError('Dump datasets failed due to the datasets format')
