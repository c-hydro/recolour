...
def read_file_tiff(file_name, file_band_default='band_{}', file_proj='EPSG:4326',
    ...
    if src is not None:
        ...
        if file_band_n is not None:
            data_select = data_array[file_band_n - 1, :, :]
            count = 1
        else:
            data_select = deepcopy(data_array)
        ...
        if count == 1:
            ...
            if data_select.ndim == 2:
                data_tmp = deepcopy(data_select)
            elif data_select.ndim == 3:
                data_tmp = data_select[0, :, :]
            else:
