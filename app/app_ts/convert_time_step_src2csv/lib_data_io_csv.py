...
def wrap_datasets_csv(file_path_template,
    ...
    if time_frequency == 'H':
        ...
        time_start = pd.Timestamp(time_start).floor(time_rounding)
        time_end = pd.Timestamp(time_end).floor(time_rounding)

        time_range_start = pd.date_range(time_start, time_end, freq=time_frequency)
        time_range_end = pd.date_range(time_start, time_end, freq=time_frequency)
        time_range_reference = pd.date_range(time_start, time_end, freq=time_frequency)
        ...
        for time_step_start, time_step_end, time_step_reference in zip(
            ...
            if os.path.exists(file_path_defined):
                ...
                fields_data_raw = pd.read_csv(
                    file_path_defined, sep=file_sep, decimal=file_decimal, date_parser=time_format)
