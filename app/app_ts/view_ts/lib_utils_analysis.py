...
def join_time_series(ts_obj_collections, ts_name_ref='ref'):
    ...
    ts_dframe_collections = pd.DataFrame(data=ts_obj_reference, index=ts_time_reference)
    ...
    for ts_name_other, ts_series_other in ts_obj_collections.items():
        ...
        if ts_series_other is not None:
            ...
            ts_dframe_collections = ts_dframe_collections.join(ts_dframe_other)
            ts_dframe_collections[ts_name_other].attrs = attrs_series_other

            attrs_collections[ts_name_other] = attrs_series_other

    ts_dframe_collections.attrs = attrs_collections
