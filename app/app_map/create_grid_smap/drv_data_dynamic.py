...
from lib_data_io_tiff import read_file_tiff, organize_file_tiff, write_file_tiff
...
class DrvData:
    ...
    def __init__(self, alg_time_now, alg_time_reference, alg_time_datasets,
        ...
        self.tag_time_period, self.tag_time_direction = 'time_period', 'time_direction'
        self.tag_time_selection = 'time_selection'
        ...
        self.time_direction_src = self.alg_datasets_src[self.tag_time_direction]
        self.time_selection_src = self.alg_datasets_src[self.tag_time_selection]
        ...
        self.time_direction_dst = self.alg_datasets_dst[self.tag_time_direction]
        self.time_selection_dst = self.alg_datasets_dst[self.tag_time_selection]
    ...
    def organize_data(self):
        ...
        for alg_time_group in self.alg_time_datasets:
            ...
            alg_time_subset = set_time_file(
                time_now=alg_time_now, time_start=alg_time_group, time_end=None,
                time_frequency=self.time_frequency_src, time_rounding=self.time_rounding_src,
                time_period=self.time_period_src, time_direction=self.time_direction_src,
                time_reverse=True)
            ...
            for alg_time_step in alg_time_subset:
                ...
                if not os.path.exists(file_path_anc_raw_step):
                    ...
                    if os.path.exists(file_path_src_step):
                        ...
                        # get datasets source
                        dset_src_step, attrs_src_step = read_file_tiff(
                            file_path_src_step, var_limit_min=0, var_limit_max=1,
                            output_format='dataset', var_name_tmpl='soil_moisture')

                        # organize variable(s) source
                        vars_src_step = organize_data(dset_src_step, var_obj_attributes=attrs_src_step,
                                                      var_name_upd=None)
                        ...
                        # time selection method to exit from the iteration(s)
                        if self.time_selection_src == 'first':
                            logging.info(' -----> Time Selection is set to "' + str(self.time_selection_src) +
                                         '". At the first occurrence the search was stopped by the algorithm. ')
                            break
                        elif self.time_selection_src == 'all':
                            pass
                        else:
                            logging.error(' ===> Time selection method is not supported')
                ...
                if not os.path.exists(file_path_anc_def_step):
                    ...
                    if os.path.exists(file_path_anc_raw_step):
                        ...
                        if fx_active_dset_cmp:
                            ...
                            # method to convert data
                            fx_obj_data_cmp = convert_data(fx_obj_data_src)
                            # info end crop datasets
                            logging.info(' ------> (1) Compute datasets ... SKIPPED')
