...
def search_idx_between_grids(
        gpi_in, lon_in, lat_in, gpi_list_out, grid_obj_out, max_dist=25000):
    ...
    if idx_out.shape[0] == 0:
        ...
        # check idx lut between grids
        if idx_out.shape[0] == 0:
            alg_logger.warning(' ===> No gpi(s) found for the location "' + str(gpi_in) + '"')
            return None
        else:
...
def get_obj_1d(cell_data, cell_coord, cell_attrs, row_start, row_end, var_min, var_max, index_name_time='time'):
...
def apply_scaling_datasets_cell(
    ...
    for idx_step_ref_nrt, (gpi_step_ref_nrt, lon_step_ref_nrt, lat_step_ref_nrt) in enumerate(
            zip(gpi_list_ref_nrt, lon_list_ref_nrt, lat_list_ref_nrt)):

        # method to search gpi between grids (reference nrt and reference dr)
        idx_step_ref_dr = search_idx_between_grids(
            gpi_step_ref_nrt, lon_step_ref_nrt, lat_step_ref_nrt, gpi_list_ref_dr, grid_obj_ref_dr)

        # method to search gpi between grids (reference nrt and other dr)
        idx_step_other_dr = search_idx_between_grids(
            gpi_step_ref_nrt, lon_step_ref_nrt, lat_step_ref_nrt, gpi_list_other_dr, grid_obj_other_dr)
        ...
        if idx_step_ref_dr is not None and idx_step_other_dr is not None:
            ...
            if row_size_cumulative_list_ref_nrt is not None:
                ...
                obj_ref_nrt, attrs_ref_nrt = get_obj_1d(
                    ...
                    row_size_start_ref_nrt, row_size_end_ref_nrt, var_min, var_max,
            ...
            if row_size_cumulative_list_ref_dr is not None:
                ...
                obj_ref_dr, attrs_ref_dr = get_obj_1d(
                    ...
                    row_size_start_ref_dr, row_size_end_ref_dr, var_min, var_max,
            ...
            alg_logger.warning(' ===> No gpi(s) found for the location reference dr "' +
                               str(idx_step_ref_dr) + '" and/or for the location other dr "' +
                               str(idx_step_other_dr) + '"')


        # search idx lut between grids
        idx_step_ref_dr = np.argwhere(gpi_list_ref_dr == gpi_step_ref_nrt)

        # check idx lut between grids
        if idx_step_ref_dr.shape[0] == 0:
            gpi_step_ref_dr = grid_obj_ref_dr.find_nearest_gpi(
                lon=lon_step_ref_nrt, lat=lat_step_ref_nrt, max_dist=max_dist)[0]
            idx_step_ref_dr = np.argwhere(gpi_list_ref_dr == gpi_step_ref_dr)

            # check idx lut between grids
            if idx_step_ref_dr.shape[0] == 0:
                alg_logger.warning(' ===> No gpi(s) found for the location "' + str(gpi_step_data) + '"')
                idx_not_found.append(gpi_step_ref_nrt)
                continue

        elif idx_step_ref_dr.shape[0] == 1:
            idx_step_ref_dr = idx_step_ref_dr.flatten()[0]
        else:
            alg_logger.error(' ===> Multiple gpi(s) found for the same location')
            raise RuntimeError('Check your grid object')
