...
def resample_grid_to_points(obj_map, obj_cell, geo_mask, geo_x_2d, geo_y_2d,
                            ...
                            var_min=0, var_max=1, var_no_data=np.nan,
                            search_rad=50000, fill_values=np.nan, min_neighbours=1, neighbours=1, debug=True):
    ...
    if obj_cell is not None:
        ...
        if var_name_dset in list(obj_map.keys()):
            ...
            var_map_obj = resample_to_grid(
                {'data': var_map_data_1d_in},
                geo_x_2d.ravel(), geo_y_2d.ravel(), var_cell_x_2d, var_cell_y_2d,
            ...
            var_cell_obj = resample_to_grid(
                ...
                var_cell_x_1d, var_cell_y_1d, var_cell_x_2d, np.flipud(var_cell_y_2d),
                search_rad=search_rad, fill_values=fill_values,
                min_neighbours=min_neighbours, neighbours=neighbours)
            var_cell_data_2d_resampled = np.flipud(var_cell_obj['data'])

            # define geometry definition(s)
            map_obj = pyresample.geometry.GridDefinition(lons=geo_x_2d, lats=geo_y_2d)
            ...
            var_map_data_1d_resampled = var_map_data_2d_in[index_array_2d[0], index_array_2d[1]]
            ...
            var_map_data_1d_resampled[:] = var_no_data
            obj_cell[var_name_data] = var_map_data_1d_resampled
...
def resample_points_to_grid(var_data_in, var_lon_1d_in, var_lat_1d_in,
                            var_mask_2d_out, var_geox_1d_out, var_geoy_1d_out,
                            search_rad=50000, fill_values=np.nan,
                            min_neighbours=1, neighbours=4):

    var_geox_2d_out, var_geoy_2d_out = np.meshgrid(var_geox_1d_out, var_geoy_1d_out)

    values_obj = resample_to_grid(
        {'data': var_data_in},
        var_lon_1d_in, var_lat_1d_in, var_geox_2d_out, var_geoy_2d_out,
        search_rad=search_rad, fill_values=fill_values,
        min_neighbours=min_neighbours, neighbours=neighbours)
