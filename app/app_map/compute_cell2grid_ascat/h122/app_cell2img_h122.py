...
- Assignment is allowed only if nearest point distance <= roi_km
- Final output is filtered using a mask generated from original mapped points
  extended by extension_pixels
- Mask is assumed to be 0/1, where 1 = valid domain and 0 = outside domain
"""
...
def main():
    ...
    try:
        (soil_moisture_map_smooth, soil_moisture_map_interp, time_lag_map,
         profile, stats,
         time_start, time_end) = process(settings, reference_time)
...
def get_args():
    ...
    parser = argparse.ArgumentParser(
        description="Convert H122 cell files to raster maps using nearest ROI + pixel mask"
    )
