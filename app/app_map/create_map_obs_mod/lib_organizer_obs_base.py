...
from lib_organizer_obs_apps import (AscatDriverNRT, AscatDriverTS,
                                    AscatDriverAnalysis, AscatDriverConversion, AscatDriverMapping)
from lib_organizer_obs_apps import AscatDriverDR as AscatDriverDR_OBS
from lib_organizer_mod_apps import AscatDriverDR as AscatDriverDR_MOD
...
class AscatOrganizerNRT:
    ...
    def composer(self):
        ...
        # iterate over cell(s)
        n = len(self.domain_cells)
        for i, cell in enumerate(self.domain_cells):

            # compute percentage
            percentage = str(np.round(100 * ((i + 1) / n), 1)) + '%'
            # cell info start
            log_stream.info(' -----> [' + percentage + '] --- Cell "' + str(cell) + '" ...')
        ...
        # compose info end
        log_stream.info(' -----> Compose data ... DONE')
