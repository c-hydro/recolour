...
from lib_utils_generic import merge_dict, check_filename

from lib_resampler_base import AscatResamplerConfigure
from lib_organizer_obs_base import AscatOrganizerNRT, AscatOrganizerDataRecord
from lib_info_args import logger_name, time_format, time_units, time_calendar
...
class DataProductWrapper:
    ...
    def __init__(self, **kwargs):
        ...
        self.filepath_data_raw_ref, self.filename_tmpl_raw_ref = self.__splitList(kwargs['sm_data_raw_ref'])
    ...
    def execDataProduct(self):
        ...
        if not check_filename(self.ascat_organizer.ascat_path_maps, self.filepath_grid_ref_domain,
            ...
            self.ascat_organizer.mapper(parameters_attrs=merge_dict(self.var_obj_in, self.var_obj_out))
    ...
    def resampleDataProduct(self, dr=False, write_n_resampled=None):
        ...
        for (ascat_product, ascat_pathname_in,
             ascat_pathname_raw, ascat_filename_raw,
             ascat_pathname_out, ascat_filename_out) in zip(
                self.var_product, self.filepath_data_in_ref,
                self.filepath_data_raw_ref, self.filename_tmpl_raw_ref,
            ...
            ascat_resampler_configure = AscatResamplerConfigure(
                time_ref,
                ascat_product,
                ascat_path_swath=ascat_pathname_in,
                ascat_path_raw=ascat_pathname_raw,
                ascat_filename_raw=ascat_filename_raw,
                ascat_path_ts=ascat_pathname_out,
