...
def get_logger(logger, settings, reference_time=None):
    ...
    level_str = settings.get("level", "INFO").upper()
    level = getattr(logging, level_str, logging.INFO)

    log_folder_template = settings.get("folder", "./log")
    log_filename_template = settings.get("filename", "cell2img_h122.log")
    rotate_daily = settings.get("rotate_daily", False)
    ...
    log_filename = resolve_time_tags(
