...
# -----------------------------------------------------------------------------------------
# Script information
script_name='SM MODEL GRID - ORGANIZER TS - AIR TEMPERATURE - HISTORY (24h)'
...
fp_settings="/hydro/fp_tools_postprocessing/analyzer_sm_cnr/model_grid/organizer_ts/sm_model_points_extractor_airt_history_24h.json"

# Lock folder/file
folder_lock="/hydro/lock/sm_cnr"
fp_lock="${folder_lock}/run_extract_points_airt_history_24h.lock"
...
print_usage() {
    ...
    echo
    echo "  --hour HH:MM"
    echo "      Hour applied to every date in history mode."
    echo "      Default: ${time_hour_default}"
    echo
    echo "  -f, --force"
...
if [ "${execution_mode}" = "realtime" ]; then
    ...
    if [[ ! "${time_hour}" =~ ^([01][0-9]|2[0-3]):[0-5][0-9]$ ]]; then
