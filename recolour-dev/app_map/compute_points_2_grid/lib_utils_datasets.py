...
def organize_datasets_cell(
    ...
    for cell_n in cell_list:
        ...
        if os.path.exists(file_path_datasets):
            ...
            # get datasets in
            cell_datasets_in = read_file_cell(file_path_datasets, selected_variables=list_variable_in)
            ...
            for var_name_in, var_name_out in zip(list_variable_in, list_variable_out):
                ...
                if var_name_in in list(cell_datasets_in.keys()):
                    ...
                    logging.warning(' ===> Variable "' + var_name_in + '" not available ih the cell datasets')
...
def organize_datasets_grid(data_obj, data_type='ASCAT', grid_dframe=None,
                           file_name_grid='TUW_WARP5_grid_info_2_3.nc'):

    # get reference grid
    if grid_dframe is None:
    ...
    if ('gpi' in data_obj) and ('gpi' in grid_dframe) and ('committed_area' in grid_dframe):
        ...
        logging.error(' ===> Grid fields are not available in the grid file "' + file_path_grid + '" ')
