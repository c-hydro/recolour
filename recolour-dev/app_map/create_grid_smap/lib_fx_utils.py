...
def organize_data(dset_obj: xr.Dataset, var_obj_attributes: dict,
                  var_name_geo_x: str = 'longitude', var_name_geo_y: str = 'latitude',
                  var_name_k: str = 'soil_moisture', var_name_attributes='attributes',
                  var_name_upd=None):
    ...
    var_data_k = select_variable(dset_obj, var_name_k, variable_mandatory=True)

    # organize variables obj
    var_obj_tmp = {var_name_geo_x: var_data_geo_x, var_name_geo_y: var_data_geo_y,
                   var_name_k: var_data_k, var_name_attributes: var_obj_attributes}
...
def convert_data(data_obj_in,
                 var_name_attributes='attributes', var_name_geo_x='longitude', var_name_geo_y='latitude',
    ...
    if var_name_attributes in list(data_obj_in.keys()):
        var_data_attributes = data_obj_in[var_name_attributes]
        data_obj_in.pop(var_name_attributes)
    else:
        logging.warning(' ===> Attributes variable "' + var_name_attributes + '" is not available in the datasets')
        var_data_attributes = None
    ...
    for var_name, var_data in data_obj_in.items():
        ...
        if var_data_attributes is not None:
            var_da.attrs = var_data_attributes
