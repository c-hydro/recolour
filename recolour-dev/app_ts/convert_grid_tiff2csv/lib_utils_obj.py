...
def filter_dataset(dset_obj, dset_vars_filter=None):
    ...
    if dset_vars_filter is not None:
        ...
        for var_name_dset in var_list_dset:
            ...
            var_attrs = dset_obj[var_name_dset].attrs
            values_attrs = list(var_attrs.values())

            for var_id, var_name_in in enumerate(var_list_in):
                if var_name_in in values_attrs:
                    var_name_out = var_list_out[var_id]
