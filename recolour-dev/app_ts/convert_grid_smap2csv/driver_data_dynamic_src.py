...
class DriverData:
    ...
    @staticmethod
    def search_file_name(file_path_raw):
        if '*' in file_path_raw:
            file_path_select = glob.glob(file_path_raw)
        else:
            file_path_select = deepcopy(file_path_raw)

        if file_path_select:
            if isinstance(file_path_select, list):
                if file_path_select.__len__() == 1:
                    file_path_select = file_path_select[0]
                else:
                    log_stream.error(' ===> File selection returned with unexpected format')
    ...
    def organize_data(self):
        ...
        if not os.path.exists(file_path_anc_def):
            ...
            for time_step in time_obj:
                ...
                file_path_src_tmp = self.define_file_name(file_path_src_raw, time_step=time_step)
                # search source file name
                file_path_src_step = self.search_file_name(file_path_src_tmp)

                # check source file availability
                if file_path_src_step:
                    ...
                    if os.path.exists(file_path_src_step):
                        ...
                        log_stream.warning(' ===> File "' + file_path_src_step + '" not found')

                else:
                    log_stream.warning(' ===> Files are not available')
